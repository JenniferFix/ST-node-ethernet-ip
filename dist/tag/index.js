...
const data_types_1 = require("../enip/cip/data-types");
const dateformat_1 = __importDefault(require("dateformat"));
const deep_equal_1 = __importDefault(require("deep-equal"));
// Static Class Property - Tracks Instances
let instances = 0;
class Tag extends events_1.EventEmitter {
    /**
     * PLC Tag Object
     *
    ...
    constructor(tagname, program = null, datatype = null, keepAlive = 0, arrayDims = 0, arraySize = 0x01) {
        ...
        if (!(0, data_types_1.isValidTypeCode)(datatype) && datatype !== null)
        ...
        let isBitString = datatype === data_types_1.Types.BIT_STRING && !isNaN(Number(pathArr[pathArr.length - 1]));
        ...
        if (isBitString) {
            ...
            pathArr[pathArr.length - 1] = ((parseInt(pathArr[pathArr.length - 1]) - bitIndex) /
                32).toString();
        }
        else {
            if (isBitIndex) {
        ...
        if (program)
            bufArr.push(enip_1.CIP.EPATH.segments.DATA.build(`Program:${program}`));
        // Build EPATH Buffer
        for (let path of pathArr) {
            bufArr.push(enip_1.CIP.EPATH.segments.DATA.build(path));
        }
     ...
     */
    get type() {
        return (0, data_types_1.getTypeCodeString)(this.state.tag.type);
    }
    /**
     * Gets Tag Bit Index
    ...
    set type(type) {
        if (typeof type === 'number') {
            if (!(0, data_types_1.isValidTypeCode)(type))
                throw new Error("Datatype must be a Valid Type Code <number>");
            this.state.tag.type = type;
        }
        else {
            if (!(0, data_types_1.isValidTypeCode)(data_types_1.Types[type]))
                throw new Error("Datatype must be a Valid Type Code <number>");
            this.state.tag.type = data_types_1.Types[type];
        }
    }
    /**
     * Gets Tag Read Size
    ...
    get value() {
        if (Array.isArray(this.state.tag.value)) {
            ...
            setTimeout(() => {
                if (!(0, deep_equal_1.default)(prevValue, this.state.tag.value))
     ...
     */
    set value(newValue) {
        if (!(0, deep_equal_1.default)(newValue, this.state.tag.value))
            this.state.tag.stage_write = true;
    ...
    set controller_value(newValue) {
        if (!(0, deep_equal_1.default)(newValue, this.state.tag.controllerValue)) {
            let lastValue = null;
            ...
            if (lastValue !== null)
                this.emit("Changed", this, lastValue);
            else
     ...
     */
    get timestamp() {
        return (0, dateformat_1.default)(this.state.timestamp, "mm/dd/yyyy-HH:MM:ss.l");
    }
    /**
    ...
    parseReadMessageResponse(data) {
        ...
        else if (type === data_types_1.Types.STRUCT) {
    ...
    parseReadMessageResponseValueForBitIndex(data) {
        const { tag } = this.state;
        const { SINT, INT, DINT, BIT_STRING, UINT } = data_types_1.Types;
        // Read Tag Value
    ...
    parseReadMessageResponseValueForAtomic(data) {
        const { SINT, INT, DINT, REAL, BOOL, LINT, BIT_STRING, UINT } = data_types_1.Types;
    ...
    generateWriteMessageRequest(value = null, size = 0x01) {
        ...
        const { tag } = this.state;
        if (tag.type === null)
            throw new Error(`Tag ${tag.name} has not been initialized. Try reading the tag from the controller first or manually providing a valid CIP datatype.`);
    ...
    generateWriteMessageRequestForBitIndex(value) {
        const { tag } = this.state;
        const { SINT, INT, DINT, BIT_STRING } = data_types_1.Types;
        // Build Message Router to Embed in UCMM
        let buf = null;
        /* eslint-disable indent */
        switch (tag.type) {
    ...
    generateWriteMessageRequestForAtomic(value, size) {
        const { tag } = this.state;
        const { SINT, INT, DINT, REAL, BOOL, LINT } = data_types_1.Types;
        // Build Message Router to Embed in UCMM
    ...
    static isValidTagname(tagname) {
        ...
        const userRegex = new RegExp("^(Program:" +
            nameRegex(3) +
